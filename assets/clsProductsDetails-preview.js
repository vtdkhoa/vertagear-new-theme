...
var clsProductsDetails = (function () {
  ...
  function cart_init() {
    ...
    // add link to Card icon tren header top 
    $('.main-header .main-header__cart').replaceWith($('.main-header .main-header__cart').clone());

    $('.main-header .main-header__cart').click(function (e) {
    ...
    observeDOM(document.querySelector('#viewreorder'), function (m) {
      ...
      if ($('.pre-order-2-button').length > 0) {
        ...
        $('#namProductAtcBtn, .buttonsaddtocart').html('ADD TO CART').removeClass('reorder');
  ...
  function cart_add() {
    ...
    // kiem tra "RGB LED Top Kit" xem chọn chua ?
    let isRgbKitAdded = ProductRgbCartData.options[0].isSelected;  //key: "upgrade",
    ...
    //console.log('rgbOptionSelected', rgbOptionSelected)
    var arrItemsProducts = []
    for (let i = 0; i < rgbOptionSelected.length; i++) {
      const o = rgbOptionSelected[i];
      arrItemsProducts.push({ quantity: ProductRgbCartData.quantity, id: o.shopifyVariantId })
    }

    //console.log('logo_getproperties', logo_getproperties)

    if (logo_getproperties) {
      arrItemsProducts.push(logo_getproperties)
    } else {
      // add san pham chinh thuc
    }

    //console.log('arrItemsProducts', arrItemsProducts)
